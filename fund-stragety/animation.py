...
def load_and_prep_data():
    # 读取数据
    df = pd.read_json('sh510300_hist_sina.json')
...
for i in range(start_idx, end_idx + 1):
    ...
    current_df['ma30_dynamic'] = current_df['close'].rolling(window=1500, center=True, min_periods=1).mean()
...
init_df['ma30_dynamic'] = init_df['close'].rolling(window=1500, center=True, min_periods=1).mean()
